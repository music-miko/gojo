...
from Curse import DEV_USERS, LOGGER, OWNER_ID, SUDO_USERS
...
anti_c_send = [-1001576075071]
anti_forward = [-1001576075071]
anti_forward_u = []
anti_forward_c = []
anti_links = []


@app.on_message(command("locktypes"))
...
@app.on_message(command("lock") & restrict_filter)
...
@app.on_message(command("locks") & restrict_filter)
...
@app.on_message(command("unlock") & restrict_filter)
...
async def is_approved_user(c: app, m: Message):
    ...
    if m.forward_from:
        if (
            ...
            or m.from_user.id == Config.BOT_ID
        ):
            return True
        return False
...
@app.on_message(filters.all & ~filters.me, 18)
async def lock_del_mess(c: app, m: Message):
    all_chats = (
        anti_c_send + anti_forward + anti_forward_c + anti_forward_u + anti_links
    )
