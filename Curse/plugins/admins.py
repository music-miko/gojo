...
# imports
# Remove the line that imports get_support_staff
# from supports import get_support_staff

def get_support_staff():
    return []  # Placeholder for now

SUPPORT_STAFF = get_support_staff()  # Keep this line to call the function
...
async def adminlist_show(_, m: Message):
    ...
    try:
        ...
        if str(ef) == str(m.chat.id):
            ...
            await m.reply_text(
                text=f"Some error occured, report it @Lux_Bot_support \n <b>Error:</b> <code>{ef}</code>"
            )
        LOGGER.error(ef)
