...
from Curse.utils.msg_types import Types, get_filter_type
from Curse.utils.regex_utils import regex_searcher
...
@app.on_message(command("filters") & filters.group & ~filters.bot)
...
@app.on_message(command(["filter", "addfilter"]) & admin_filter & ~filters.bot)
...
@app.on_message(command(["stop", "unfilter"]) & admin_filter & ~filters.bot)
async def stop_filter(_, m: Message):
    ...
    keyword_to_stop = m.text.split(None, 1)[1].lower()  # Get the keyword to stop
    if keyword_to_stop in act_filters:
        db.rm_filter(m.chat.id, keyword_to_stop)
        LOGGER.info(
            f"{m.from_user.id} removed filter ({keyword_to_stop}) in {m.chat.id}"
        )
        await m.reply_text(
            f"Okay, I'll stop replying to that filter and its aliases in <b>{m.chat.title}</b>.",
        )
        await m.stop_propagation()
    else:
        await m.reply_text(
