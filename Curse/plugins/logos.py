...
async def lego(client, message):
    quew = message.text.split("/logo ")[-1]

    if message.from_user.id != OWNER_ID and not quew:
        await message.reply("Please give me a text for the logo.")
        return

    pesan = await message.reply("Generating logo. Please wait...")

    try:
        text = message.text.split("/logo ")[-1]

        randc = random.choice(LOGO_LINKS)

        response = await fetch.get(randc)
        ...
        fnt = glob.glob("./Curse/extras/Logo/*")
