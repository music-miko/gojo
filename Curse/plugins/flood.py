...
from Curse.utils.extras import BAN_GIFS, KICK_GIFS, MUTE_GIFS
...
close_kb = InlineKeyboardMarkup(
    [[InlineKeyboardButton("Close ❌", callback_data="f_close")]]
)

action_kb = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton("Mute 🔇", callback_data="f_mute"),
            InlineKeyboardButton("Ban 🚷", callback_data="f_ban"),
            InlineKeyboardButton("Kick 🦿", callback_data="f_kick"),
        ],
        [InlineKeyboardButton("➡️ Skip", callback_data="f_skip")],
    ]
)
...
within_kb = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton("5", callback_data="f_f_5"),
            InlineKeyboardButton("10", callback_data="f_f_10"),
            InlineKeyboardButton("15", callback_data="f_f_15"),
        ],
        [InlineKeyboardButton("➡️ Skip", callback_data="f_f_skip")],
    ]
)
...
limit_kb = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton("5", callback_data="f_5"),
            InlineKeyboardButton("10", callback_data="f_10"),
            InlineKeyboardButton("15", callback_data="f_15"),
        ],
        [InlineKeyboardButton("➡️ Skip", callback_data="f_f_f_skip")],
    ]
)


@app.on_message(command(["floodaction", "actionflood"]) & admin_filter)
async def flood_action(c: app, m: Message):
    Flood = Floods()
...
@app.on_message(command(["isflood", "flood"]) & ~filters.bot)
async def flood_on_off(c: app, m: Message):
    if m.chat.type == CT.PRIVATE:
...
@app.on_message(command(["setflood"]) & ~filters.bot & admin_filter)
async def flood_set(c: app, m: Message):
    bot = await c.get_chat_member(m.chat.id, Config.BOT_ID)
    Flood = Floods()
    status = bot.status
    if not status in [CMS.OWNER, CMS.ADMINISTRATOR]:
...
async def callbacks(c: app, q: CallbackQuery):
    data = q.data
    if data == "f_close":
    ...
    if user in SUPPORT_STAFF or user_status in [CMS.OWNER, CMS.ADMINISTRATOR]:
        if data in ["f_mute", "f_ban", "f_kick", "f_skip"]:
            ...
            try:
                ...
                await q.edit_message_text(
                    "Flood protection setting has been updated", reply_markup=close_kb
                )
                return
                await q.answer("skip")
...
async def flood_watcher(c: app, m: Message):
    c_id = m.chat.id
    ...
    try:
        ...
        dic[c_id].update(
            z
        )  # make the dic something like {c_id : {u_id : [[for time],[for msg]]}}
