...
from Curse import LOGGER, TIME_ZONE
...
from Curse.utils.custom_filters import admin_filter, command, restrict_filter
...
@app.on_message(
    command(["warn", "swarn", "dwarn"]) & restrict_filter, group=90091
)
...
@app.on_message(command("resetwarns") & restrict_filter, group=90092)
...
@app.on_message(command("warns") & filters.group, group=90093)
...
@app.on_message(
    command(["rmwarn", "removewarn"]) & restrict_filter, group=90094
)
...
@app.on_message(command(["warnings", "warnsettings"]) & admin_filter, group=90095)
...
@app.on_message(command("warnmode") & admin_filter, group=90096)
...
@app.on_message(command("warnlimit") & admin_filter, group=90097)
