...
from pyrogram.enums import ChatMemberStatus as CMSG
from pyrogram.enums import ChatType
from pyrogram.errors import RPCError, UserNotParticipant
from pyrogram.filters import create
from pyrogram.types import CallbackQuery, Message

from Curse import DEV_USERS, OWNER_ID, SUDO_USERS
        ...
        if matches:
            ...
            if bool(m.chat and m.chat.type in {ChatType.SUPERGROUP}):
                try:
                    ...
                    # i.e anon admin
                    user_status = CMSG.ADMINISTRATOR
                except ValueError:
                    # i.e. PM
                    user_status = CMSG.OWNER
...
async def restrict_check_func(_, __, m: Message or CallbackQuery):
    ...
    if user and user.status in [CMSG.ADMINISTRATOR, CMSG.OWNER] and user.privileges.can_restrict_members:
        status = True
    else:
        status = False
...
async def promote_check_func(_, __, m):
    ...
    if user.status in [CMSG.ADMINISTRATOR, CMSG.OWNER] and user.privileges.can_promote_members:
        status = True
    else:
        status = False
...
async def changeinfo_check_func(_, __, m):
    ...
    if user.status in [CMSG.ADMINISTRATOR, CMSG.OWNER] and user.privileges.can_change_info:
        status = True
    else:
        status = False
...
async def can_pin_message_func(_, __, m):
    ...
    if user.status in [CMSG.ADMINISTRATOR, CMSG.OWNER] and user.privileges.can_pin_messages:
        status = True
    else:
        status = False
